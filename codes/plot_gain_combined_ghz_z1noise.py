...
def gamma_variance(p, R, dimension):
    """Standard-shadow/CRM variance ratio for GHZ under Z_1 noise."""
    numerator = dimension * (2.0 - p**2) - 2.0 * (1.0 - p) ** 2
    denominator = (
        (3.0 * dimension + 2.0) * p**2
        + 4.0 * (dimension + 1.0) * p * (1.0 - p) / R
    )
    return numerator / denominator


def gamma_total(p, x, R, dimension):
    return gamma_variance(p, R, dimension) * (1.0 + x) / (R + x)


def optimal_reuse(p, x, dimension):
    return np.sqrt(
        4.0 * (dimension + 1.0) * (1.0 - p) * x
        / ((3.0 * dimension + 2.0) * p)
    )
...
eps_inv_min = 1.0
eps_inv_max = 10**4.15
eps_inv_vals = np.logspace(0.0, 4.15, 501)

EI, Rl = np.meshgrid(eps_inv_vals, R_values, indexing="xy")
p_left = 1.0 / EI
Gl = gamma_total(p_left, x_fixed, Rl, d)

p_left_curve = 1.0 / eps_inv_vals
Rop_l = optimal_reuse(p_left_curve, x_fixed, d)
...
Gr = gamma_total(eps_fixed, Xg, Rr, d)
Rop_r = optimal_reuse(eps_fixed, xv, d)
...
print(f"n = {n}, d = {d}")
print(f"Gl range: {np.nanmin(Gl):.4e} to {np.nanmax(Gl):.4e}")
print(f"Gr range: {np.nanmin(Gr):.4e} to {np.nanmax(Gr):.4e}")
...
axL.set_xscale("log")
axL.set_yscale("log")
axL.set_xlim(eps_inv_min, eps_inv_max)

axL.set_xlabel(r"$\epsilon^{-1}$", labelpad=1)
axL.set_ylabel(r"$R$", labelpad=0)
...
Gl_min = np.nanmin(Gl)
Gl_max = np.nanmax(Gl)
vl = [lv for lv in levels if Gl_min < lv < Gl_max]
...
if vl:
    ...
    for lv in vl:
        ...
        for j in range(len(R_values) - 1):
            if not (np.isfinite(g_col[j]) and np.isfinite(g_col[j + 1])):
                continue
...
axL.text(
    label_x_left_opt,
    8e4,
    r"$R_{\rm opt}=\sqrt{\frac{4(d+1)(1-\epsilon)x}{(3d+2)\epsilon}}$",
    rotation=0,
    ha="center",
    va="top",
    fontsize=28.6,
...
Gr_min = np.nanmin(Gr)
Gr_max = np.nanmax(Gr)
vr = [lv for lv in levels if Gr_min < lv < Gr_max]
...
axL.get_xticklabels()[-1].set_horizontalalignment("right")
axR.get_xticklabels()[0].set_horizontalalignment("left")


fig.canvas.draw()
tight_bbox = fig.get_tightbbox(fig.canvas.get_renderer())
side_pad_inches = 0.005
top_pad_inches = 0.10
save_bbox = Bbox.from_extents(
    tight_bbox.x0 - side_pad_inches,
    tight_bbox.y0 - side_pad_inches,
    tight_bbox.x1 + side_pad_inches,
    tight_bbox.y1 + top_pad_inches,
)

fig.savefig(
    out_dir / f"gain_combined_ghz_z1noise_n{n}.pdf",
    dpi=300,
...
fig.savefig(
    out_dir / f"gain_combined_ghz_z1noise_n{n}.png",
    dpi=300,
    bbox_inches=save_bbox,
    pad_inches=0.0,
)
